...
6. animation translate
    ...
    `tick`을 이용해서 animation을 원하는 대로 만들 수 있다.
    ```
...
- Unit Test
    - 단일 테스트에 대한 심화 작성
- View Rendering을 위한 Framework
    - Files ViewFrameWork
    - 최근 기술 `Thymeleaf` 2.xx 버전 (안정화 성능 향상)
- Transactional에 대한 처리
...
2. queryForObject를 params 없이 사용하는 방법
```java
...
4. 데이터가 많을 때 이중포문 보다 포문을 두번 돌리는 것이 빠르다.
...
13. HttpServletRequest, HttpServletResponse는 최대한 숨긴다.
- 이유
...
```
- 반복되는 코드를 Custom Class로 확장해서 사용 할 수 있다.

16. Rest API를 사용하는 경우
```js
HttpStatus.Code = {
    200 : 성공
